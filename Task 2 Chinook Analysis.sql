...
SELECT c.country, COUNT(DISTINCT c.customer_id) AS unique_buyers, COUNT(i.invoice_id) AS number_of_invoices,
...
-- Monthly Performance 
SELECT DATE_TRUNC('month', i.invoice_date) AS month, SUM(i.total) AS monthly_revenue,
...
SELECT ar.name As artist_name, SUM (il.unit_price * il.quantity) AS revenue
FROM artist ar
...
SELECT c.city, SUM(i.total) AS total_revenue, COUNT(DISTINCT c.customer_id) AS unique_customers
FROM customer c
JOIN invoice i ON c.customer_id = i.customer_id
GROUP BY c.city
ORDER BY total_revenue DESC;
